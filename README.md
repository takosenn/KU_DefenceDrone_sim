...
上記の文章はAiに作成させた文章．ここからは前年度の卒業生からのメッセージです．
- この研究はドローンを用いた警戒網システムの実現を目指しました．
- Python3.13とCoppeliasim4.10.0を使用
- 制御式は添付したURLの論文をそのまま流用したのでコードが気に入らない場合は論文を見て自分でコードを組みなおしてください．それほど難しくなさそうです．
- 実機までいきたかったがHulaドローンの複数機の接続がうまくできず，断念した．単機の接続は可能です．ただしPython3.6とかいう古いverなので期待はできない．
- シミュレータのドローンの追従性能を考えてドローンの速度の限界値を制限しないと制御入力が大きすぎて暴れてしまう．
- 角速度を切り替える時にMax速度を変えている理由はそのほうが結果のグラフがきれいになったから．同じMax速度でも収束はします．
- Coppeliasimのシミュレーションシーンは添付した.tttファイルです．もしかしたらGitHubでうまくcommitできていないかも？その場合はドローン（Quadcopter）を7台追加して位置につかせてください．
